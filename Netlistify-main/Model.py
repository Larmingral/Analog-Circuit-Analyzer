...
def set_seed(seed, deterministic):
    if deterministic:
        # [CPU MODIFIED] 移除 CUDA 相关的确定性设置
        # os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
        torch.use_deterministic_algorithms(True, warn_only=True)
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    # [CPU MODIFIED] 移除 cudnn 设置
...
class Model:
    ...
    def __init__(
            ...
            shuffle=False,
            amp=False,  # [CPU MODIFIED] 默认禁用混合精度
            cudnn_benchmark=False,
            cudalize=False,  # [CPU MODIFIED] 默认不进行 cudalize
            use_cache=True,
            memory_fraction=1,
            eval=False,
            log2console=True,
        ...
        # [CPU MODIFIED] 默认 transform 改为 cpu()
        ...
        self.amp = False  # [CPU MODIFIED] 强制禁用 AMP

        # [CPU MODIFIED] 移除 cudnn
        # if cudnn_benchmark:
        #     torch.backends.cudnn.enabled = True
        #     torch.backends.cudnn.benchmark = True

        self.writer = None
        # torch.set_float32_matmul_precision("high") # [CPU MODIFIED] 可选移除
    ...
    def preprocessing(self, data: Datasetbehaviour, use_cache, cudalize):
        ...
        if cudalize:
            # [CPU MODIFIED] cudalization 现在实际上是 cpu-alization
            self.print("*processing (cpu)")
    ...
    def fit(
        ...
        # [CPU MODIFIED] 移除 CUDA_VISIBLE_DEVICES
        ...
        if pretrained_path:
            ...
            # [CPU MODIFIED] 强制 map_location='cpu'
        ...
        try:
            ...
            def create_meta(seq, ep, mode) -> MetaData:
                return MetaData(
                    ...
                    # [CPU MODIFIED] DataParallel 处理移除，直接用 self.model
                    model=self.model,
                    epoch=ep,
                    mode=mode,
                )
                ...
                with tqdm(
                    ...
                    try:
                        ...
                        for batch_num, (seq, data, target) in enumerate(self.train_loader):
                            ...
                            # [CPU MODIFIED] 移除 autocast 和 scaler
                            ...
                            # [CPU MODIFIED] 直接 backward，不用 scaler
                            (loss / backprop_freq).backward()
                            train_loss.append(loss.item())

                            if (batch_num + 1) % backprop_freq == 0:
                                # [CPU MODIFIED] 直接 step
                                optimizer.step()
                                optimizer.zero_grad()
                    ...
                    if mean_train_loss < best_train_loss:
                        ...
                        # [CPU MODIFIED] 移除 scaler 传参 (用 None 代替)
                        self.save_model(saved_path, model, device_ids, optimizer, None, ep, False)
    ...
    def save_model(self, path, model, device_ids, optimizer, scaler, ep, force):
        state = {
            # [CPU MODIFIED] 不再检查 device_ids 长度，直接保存 model.state_dict()
            "model": model.state_dict(),
            "optimizer": optimizer.state_dict(),
            "epoch": ep,
        }
        # [CPU MODIFIED] 仅当 scaler 不为 None 时才保存
    ...
    def gc(self):
        ...
        # [CPU MODIFIED] 移除 cuda empty cache
